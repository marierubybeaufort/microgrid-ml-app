...
with tab6:
    ...
    # ===================== Data Setup (IESO) =====================
# Build all app inputs from a raw IESO "Hourly Consumption by FSA" CSV
# Non-destructive to the rest of your app. Only writes to /data/*.csv
with st.expander("Data Setup (IESO) — build app inputs from raw IESO FSA CSV", expanded=False):
    st.write("Point to your downloaded file (e.g., `data/PUB_HourlyConsumptionByFSA_202505_v1.csv`).")
    raw_path = st.text_input(
        "Raw IESO FSA CSV path",
        value="data/PUB_HourlyConsumptionByFSA_202505_v1.csv",
        help="Use the monthly CSV you downloaded from IESO (Hourly Consumption by Forward Sortation Area).",
    )
    out_dir = "data"

    def _read_ieso_fsa_csv(path: str) -> pd.DataFrame | None:
        if not os.path.exists(path):
            st.warning(f"File not found: {path}")
            return None
        # Some IESO files have a preamble. Find the header line that starts with 'FSA,'
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                lines = f.read().splitlines()
            header_idx = None
            for i, line in enumerate(lines[:200]):
                if line.strip().upper().startswith("FSA,"):
                    header_idx = i
                    break
            if header_idx is None:
                st.error("Could not locate CSV header line starting with 'FSA,'.")
                return None
            import io
        ...
        need = {"FSA", "DATE", "HOUR", "TOTAL_CONSUMPTION"}
        if not need.issubset(set(df.columns)):
            st.error(f"CSV missing columns {sorted(list(need))}. Found: {list(df.columns)}")
            return None

        # Build timestamp (hour-ENDING convention 0..24 handled simply here)
        df["timestamp"] = pd.to_datetime(df["DATE"], errors="coerce") + pd.to_timedelta(
            df["HOUR"].clip(0, 24), unit="h"
        )
        # Convert MWh -> kW for 1-hour interval
        df["kw"] = df["TOTAL_CONSUMPTION"].astype(float) * 1000.0
        return df

    def _build_from_df(df_wide: pd.DataFrame, out_dir: str) -> dict:
        # Choose Ottawa student FSAs: K1N (uOttawa), else K1S (Carleton), else any K1*, else first available
        fsas = set(df_wide["FSA"].astype(str).str.upper().str.strip().unique())
        chosen = None
        for p in ["K1N", "K1S"]:
            if p in fsas:
                chosen = p
                break
        if chosen is None:
            k1_like = [f for f in fsas if f.startswith("K1")]
            chosen = k1_like[0] if k1_like else list(fsas)[0]

        df_fsa = (
            df_wide[df_wide["FSA"].astype(str).str.upper().str.strip() == chosen]
            .groupby("timestamp", as_index=False)["kw"].sum()
            .sort_values("timestamp")
        )

        if df_fsa.empty:
            raise ValueError("Selected FSA produced no rows.")

        # Build a 15-home neighborhood with the same *shape* scaled to ~15 kW average total
        shape = df_fsa["kw"] / max(df_fsa["kw"].mean(), 1e-9)
        target_avg_kw = 15.0  # ~1 kW/home avg
        neigh_kw = shape * target_avg_kw

        rng = np.random.default_rng(42)
        weights = rng.dirichlet(alpha=np.ones(15))
        house_ids = [f"home_{i+1:02d}" for i in range(15)]

        rows = []
        for ts, total_kw in zip(df_fsa["timestamp"].values, neigh_kw.values):
            parts = np.maximum(total_kw * weights, 0.0)
            for hid, kw in zip(house_ids, parts):
                rows.append((pd.Timestamp(ts), hid, float(kw)))
        hh = pd.DataFrame(rows, columns=["timestamp", "household_id", "generation_kw"])

        # Aggregates for the app
        agg = hh.groupby("timestamp", as_index=False).agg(
            total_generation_kw=("generation_kw", "sum"),
            avg_generation_kw=("generation_kw", "mean"),
        )
        agg["households_faulty"] = 0

        # Neighborhood load for Scheduling tab
        load_df = agg[["timestamp", "total_generation_kw"]].rename(columns={"total_generation_kw": "load_kw"})

        # Simple synthetic solar forecast aligned to timestamps (bell curve per day)
        import math
        def synthetic_solar(ts_series, peak_kw=12.0, sunrise=6, sunset=20):
            ...
            for ts in ts_series:
                ...
                if d != cur_day:
                    cur_day = d
                    daily_jitter = rng.uniform(0.75, 1.15)
                h = ts.hour
                if h < sunrise or h > sunset:
                    val = 0.0
                else:
                    x = (h - sunrise) / (sunset - sunrise) * math.pi  # map to 0..π
                    val = max(0.0, math.sin(x)) * peak_kw * daily_jitter
                out.append(val)
            return pd.Series(out, index=ts_series)

        forecast_df = pd.DataFrame(
            {"timestamp": agg["timestamp"], "generation_kw": synthetic_solar(agg["timestamp"])}
        )

        # Faults: inject 2 midday sharp drops into the solar curve
        faults = forecast_df.copy()
        faults["fault"] = 0
        midday_idx = faults[(faults["timestamp"].dt.hour >= 11) & (faults["timestamp"].dt.hour <= 15)].index
        if len(midday_idx) >= 10:
            pick = rng.choice(midday_idx, size=2, replace=False)
            for p in pick:
                window = range(p, min(p + 3, len(faults)))
                faults.loc[window, "generation_kw"] *= 0.35
                faults.loc[p, "fault"] = 1

        # Write outputs your app already consumes
        os.makedirs(out_dir, exist_ok=True)
        paths = {
            "community_households.csv": os.path.join(out_dir, "community_households.csv"),
            "community_agg.csv": os.path.join(out_dir, "community_agg.csv"),
            "load.csv": os.path.join(out_dir, "load.csv"),
            "forecast.csv": os.path.join(out_dir, "forecast.csv"),
            "faults.csv": os.path.join(out_dir, "faults.csv"),
        }
        hh.to_csv(paths["community_households.csv"], index=False)
        agg.to_csv(paths["community_agg.csv"], index=False)
        load_df.to_csv(paths["load.csv"], index=False)
        forecast_df.to_csv(paths["forecast.csv"], index=False)
        faults.to_csv(paths["faults.csv"], index=False)

        return {
            "chosen_fsa": chosen,
            "period_start": str(df_fsa["timestamp"].min()),
            "period_end": str(df_fsa["timestamp"].max()),
            "outputs": paths,
        }

    if st.button("Build app data from IESO CSV", type="primary"):
        df_wide = _read_ieso_fsa_csv(raw_path)
        if df_wide is not None:
            try:
                summary = _build_from_df(df_wide, out_dir)
                st.success(
                    f"Built app inputs from FSA **{summary['chosen_fsa']}** "
